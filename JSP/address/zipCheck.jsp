...
<script>
	...
	function send(code, sido, gugun, dong, bunji){
		...
		opener.document.frm.zipcode.value=code;	//우편번호에 뿌린다
		opener.document.frm.addr.value=address; //주소에 뿌린다
		self.close();	//자기창 닫기
	}
</script>
</head>
...
<form action="zipCheck.jsp" id="frm">
	<table>
		<tr>
			<td>동이름입력 : <input type="text" name="dong">
		...
		%>
				<tr>
					<td><a href="javascript:send('<%=zip%>','<%=sido%>',
					'<%=gugun%>','<%=d%>','<%=bunji%>')">
